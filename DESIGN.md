...
**PR#16 4巡目 nit-b**: `.audio-status-hint` only renders while the `AudioContext` hasn't yet resumed (checked on a `500ms` poll — see 聴覚 above), so it comes and goes. It lives in `.message-panel-controls`, the `auto`-sized column of `.message-panel`'s `1fr auto` grid, so its appearing/disappearing used to change that column's width, which pushed `h1`'s own `1fr` column narrower or wider, which could change how many lines `h1` wrapped to, which changed the message panel's height, which changed how much area the tile grid below got — the exact same "unrelated UI element reflows the grid" chain must-E/must-F fixed for font-size settings, but triggered by AudioContext timing instead (**PR#16 4巡目 nit-a**'s `320×568` "ホーム＋取り消し" `22.7px→21.8px` label shrink was this, not a font-size bug). `.message-panel-controls` was given a `min-width` matching `.audio-status-hint`'s own `max-width` (`200px` base, `128px` at the narrow breakpoint) unconditionally, so the column's width — and therefore everything downstream of it — no longer depends on whether the hint happens to be visible at measurement time.

**PR#16 5巡目 should-A (attempted removal, then reverted)**: the 5巡目 review asked to drop the unconditional `min-width` and instead overlay `.audio-status-hint` on top of the caregiver button via `position: absolute` (`pointer-events: none` retained), so the controls column's width would track the button alone. Two overlap bugs surfaced when this was tried literally (no width reservation at all): (1) without an explicit `width`, the hint's shrink-to-fit sizing was bounded by its own (button-only-wide, ~46px) containing block, squeezing it into 133–144px of wrapped text that overflowed past the message panel into the grid below; adding an explicit `width` (matching the old `max-width`) fixed that, but then (2) that fixed-width, `right:0`-anchored box spilled *left* past the narrow controls column into `h1`'s own grid column, genuinely overlapping `h1` text at `390×844` — confirmed as a real, visible overlap (both share the panel's visible area, nothing clips them apart), unlike the hint-vs-tile-grid case below. Since `h1` and `.message-panel-controls` are separate CSS Grid columns, only a `min-width` on the controls column structurally guarantees the hint never crosses into `h1`'s territory — so `min-width: 200px`/`128px` was kept. `position: absolute` was kept too (that part of should-A does hold: an absolutely-positioned element doesn't contribute to its container's intrinsic height, so the hint's appearing/disappearing no longer changes row height — the actual "nit-b の回帰" the review was guarding against), with an explicit `top: 44px` (not `top: 100%`, which would reference the *possibly min-height-inflated* height of its own containing block rather than the button's actual height — a bug caught mid-round when a min-height fix, below, was briefly added to the same element).

Separately, the coordinator's own stricter audit script checks that no descendant (including `.audio-status-hint`) geometrically overflows `.message-panel`'s own bottom edge — which the absolutely-positioned, unreserved-height hint does trip at narrow-landscape heights, even though `document.elementFromPoint` confirms the overflow is invisible in practice (`.message-panel`'s `overflow-y: auto` clips it; the tile grid below shows through, not the hint). Rather than resurrect a fixed reservation for this on every screen, `.message-panel:not(:has(.emergency-details, .emergency-sub)) .message-panel-controls { min-height: 84px }` reserves it only on screens where emergency content isn't competing for the same `40dvh` height budget — an unconditional reservation caused `emerg5+sub` (5 accumulated emergency lines + a sub-message) to clip the emergency sub-message instead, which is the higher-priority failure. The project's own e2e no-overlap check was also rewritten to use `document.elementFromPoint` at the overlap region's center rather than pure rect intersection, since geometric-only checks produce false positives for anything clipped by an `overflow: auto` ancestor.
